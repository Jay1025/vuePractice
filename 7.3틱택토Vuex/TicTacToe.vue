<template>
  <div>
    <div>{{ turn }}님의 턴입니다.</div>
    <table>
      <tr v-for="(rowData, rowIndex) in tableData" :key="rowIndex">
        <td @click="onClickTd(rowIndex, cellIndex)" v-for="(cellData, cellIndex) in rowData" :key="cellIndex">{{ cellData }}</td>
      </tr>
    </table>
    <div v-if="winner">{{ winner }}님의 승리</div>
  </div>
</template>

<script>
import { mapState } from 'vuex'
import store, { CLICK_CELL, SET_WINNER, RESET_GAME, CHANGE_TURN, NO_WINNER } from './store'

export default {
  store,
  data() {
    return {
      data: 1
    }
  },
  computed: {
    ...mapState(['winner', 'turn', 'tableData'])
    //2번.
    // ...mapState({
    //     winner: state => state.winner,
    //     turnState: 'turn'
    // })
    //3번.
    // ...mapState({
    //     winner(state){
    //         return state.winner + this.data
    //     },
    //     trunState: 'turn'
    // })
    // 1번.
    // winner() {
    //   return this.$store.state.winner
    // },
    // turn() {
    //   return this.$store.state.turn
    // }
  },
  methods: {
    onClickTd(rowIndex, cellIndex) {
      if (this.cellData) return

      this.$store.commit(CLICK_CELL, { row: rowIndex, cell: cellIndex }) //mutation은 $store.commit으로 실행한다. mutation이름이 CLICK_CELL인것이 실행된다
      //this.$set(this.tableData[rowIndex], cellIndex, this.turn)

      let win = false
      if (this.tableData[rowIndex][0] === this.turn && this.tableData[rowIndex][1] === this.turn && this.tableData[rowIndex][2] === this.turn) {
        win = true //가로
      }
      if (this.tableData[0][cellIndex] === this.turn && this.tableData[1][cellIndex] === this.turn && this.tableData[2][cellIndex] === this.turn) {
        win = true //세로
      }
      if (this.tableData[0][0] === this.turn && this.tableData[1][1] === this.turn && this.tableData[2][2] === this.turn) {
        win = true //대각선(1)
      }
      if (this.tableData[0][2] === this.turn && this.tableData[1][1] === this.turn && this.tableData[2][0] === this.turn) {
        win = true //대각선(2)
      }

      if (win) {
        //이긴 경우: 3줄 달성
        this.winner = this.turn
        this.$store.commit(SET_WINNER, this.turn) // 첫번째는 mutation이름, 두번째는 data이름
        this.$store.commit(RESET_GAME)
      } else {
        //지거나 무승부
        let all = true //all의 true면 무승부라는 뜻
        this.tableData.forEach((row) => {
          //무승부 검사
          row.forEach((cell) => {
            if (!cell) {
              all = false
            }
          })
        })
        if (all) {
          //무승부
          this.$store.commit(NO_WINNER)
          this.$store.commit(RESET_GAME)
        } else {
          this.$store.commit(CHANGE_TURN)
        }
      }
    }
  }
}
</script>

<style>
table {
  border-collapse: collapse;
}
td {
  border: 1px solid black;
  width: 40px;
  height: 40px;
  text-align: center;
}
</style>
